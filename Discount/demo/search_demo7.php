  ...
  //  $pieces[1] = preg_replace("#[^0-9a-z]#i"," ",$pieces[1]);
   ...
   if($b == 0 and $p == 0 and $s == 1 and $v == 0){
    ...
    if($b ==1 and $p == 0 and $s == 1 and $v == 0){
        $output .= '<a href="http://localhost/internet_plan.php?brand='. $burl . '&speed='. $surl.'">'.$bname.' '.$sname.'</a>'; 
    }
    if($b ==1 and $p == 0 and $s == 0 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?brand='. $burl . '&validity='. $vurl.'">'.$bname.' '.$vname.'</a>'; 
    }
    if($b ==0 and $p == 1 and $s == 1 and $v == 0){
        $output .= '<a href="http://localhost/internet_plan.php?speed='. $surl . '&plan='. $purl.'">'.$pname.' '.$sname.'</a>'; 
    }
    if($b ==0 and $p == 1 and $s == 0 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?validity='. $vurl . '&plan='. $purl.'">'.$pname.' '.$vname.'</a>'; 
    }    
    if($b ==0 and $p == 0 and $s == 1 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?validity='. $vurl . '&speed='. $surl.'">'.$sname.' '.$vname.'</a>'; 
    }     
    
   
    if($b ==1 and $p == 1 and $s == 1 and $v == 0){
        $output .= '<a href="http://localhost/internet_plan.php?brand='. $burl . '&plan='. $purl. '&speed='. $surl.'">'.$bname.' '.$pname.'  '.$sname.'</a>'; 
    }
    
    if($b ==1 and $p == 1 and $s == 0 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?speed='. $surl . '&plan='. $purl. '&validity='. $vurl.'">'.$bname.' '.$pname.' '.$vname.'</a>'; 
    }    
       
    if($b ==0 and $p == 1 and $s == 1 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?validity='. $vurl . '&plan='. $purl. '&speed='. $surl.'">'.$pname.' '.$sname.' '.$vname.'</a>'; 
    }   
    
    if($b ==1 and $p == 0 and $s == 1 and $v == 1){
        $output .= '<a href="http://localhost/internet_plan.php?speed='. $surl . '&validity='. $vurl. '&brand='. $burl.'">'.$bname.' '.$sname.' '.$vname.'</a>'; 
    }    
